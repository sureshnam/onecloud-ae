...
get_user_account_info() {
  ...
  ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no  sdp@100.83.160.55 "sudo mkdir -p ${WORKSPACE}"
  ...
  scp -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no  ${WORKSPACE}/env sdp@100.83.160.55:${WORKSPACE}/${BUILD_NUMBER}.env 
  ...
  scp -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no  ${WORKSPACE}/${AE_AUTOMATION_REPO_NAME}/scripts/onecloud-infra.sh sdp@100.83.160.55:${WORKSPACE}/ 
  ...
  ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no  sdp@100.83.160.55 "source ${WORKSPACE}/${BUILD_NUMBER}.env && source ${WORKSPACE}/onecloud-infra.sh && save_user_account_info && rm ${WORKSPACE}/${BUILD_NUMBER}.env"
  echo ""
  ...
  scp -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no  sdp@100.83.160.55:${WORKSPACE}/USER_INFO_${BUILD_NUMBER}.txt  ${WORKSPACE}/ 
  echo ""
  ...
  scp -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no  sdp@100.83.160.55:${WORKSPACE}/USER_INFO_${BUILD_NUMBER}.authorized_keys  ${WORKSPACE}/ 
  echo ""
